...
class _DebugActionCards extends ConsumerWidget {
    ...
    return Column(
      children: [
        ...
        _DebugCard(
          ...
          isDangerous: false,
          onTap: () => Navigator.of(context).push(
            MaterialPageRoute(builder: (context) => DriftDbViewer(scheduleDb)),
          ),
        ),
        const SizedBox(height: 12),
        _DebugCard(
  ...
  Future<void> _showClearDatabaseDialog(BuildContext context) async {
    ...
    if (confirmed == true && context.mounted) {
      ...
      await scheduleDb.transaction(() async {
        await scheduleDb.customStatement('PRAGMA foreign_keys = OFF;');
        for (final table in scheduleDb.allTables) {
          await scheduleDb.delete(table).go();
        }
      });
